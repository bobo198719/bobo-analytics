...
import { DollarSign, Percent, Calculator, Globe, Clock, Settings, Plus, Trash2, Zap, AlertCircle } from 'lucide-react';
        ...
        <div className="space-y-12 animate-in fade-in duration-700 pb-32 font-['Plus_Jakarta_Sans']">
            ...
            <header className="bg-white/5 border border-white/10 p-10 rounded-[48px] relative overflow-hidden group shadow-2xl">
                <div className="relative z-10">
                    ...
                    <p className="text-white/30 text-[10px] font-black uppercase tracking-[0.4em] mt-3 italic leading-none">Global Revenue Controller | Compliant ISO Nodes Active</p>
                </div>
            </header>
                    ...
                    <section className="bg-white/5 border border-white/10 rounded-[48px] p-10 shadow-2xl relative overflow-hidden">
                        ...
                        <div className="p-8 bg-emerald-500/5 rounded-3xl border border-emerald-500/10 mt-6 group">
                            <div className="flex items-center justify-between mb-6">
                                ...
                                <div className={`px-4 py-1.5 rounded-full text-[8px] font-black uppercase italic tracking-widest ${isoFeeMode === 'pass_on' ? 'bg-emerald-500 text-black' : 'bg-white/10 text-white/40'}`}>REVENUE COMPLIANT</div>
                            </div>
                            <div className="grid grid-cols-2 gap-4">
                                <button 
                                    onClick={() => setIsoFeeMode('pass_on')}
                                    className={`p-6 rounded-2xl border transition-all text-left group ${isoFeeMode === 'pass_on' ? 'bg-emerald-600 border-emerald-400 text-white' : 'bg-white/5 border-white/10 text-white/40'}`}
                                >
                                ...
                                <button 
                                    onClick={() => setIsoFeeMode('absorb')}
                                    className={`p-6 rounded-2xl border transition-all text-left group ${isoFeeMode === 'absorb' ? 'bg-emerald-600 border-emerald-400 text-white' : 'bg-white/5 border-white/10 text-white/40'}`}
                                >
                        ...
                        <div className="space-y-4">
                            {discounts.map(d => (
                                <div key={d.id} className="p-6 bg-white/5 rounded-3xl border border-white/10 group hover:border-indigo-500/30 transition-all flex items-center justify-between">
                                    <div className="flex items-center gap-6">
                                        <div className={`w-12 h-12 rounded-2xl flex items-center justify-center font-black italic text-lg ${d.active ? 'bg-indigo-500/20 text-indigo-400 border border-indigo-500/30' : 'bg-white/5 text-white/20'}`}>
                                        ...
                                        <div>
                                            ...
                                            <div className="flex items-center gap-4 mt-2">
                                                ...
                                                <span className="text-[7px] font-black uppercase text-white/20 italic tracking-widest">{d.active ? 'ACTIVE NODE' : 'INACTIVE'}</span>
                                            </div>
                                        </div>
                                    </div>
                                    <div className="flex items-center gap-6">
                                        <span className="text-xl font-black italic text-indigo-400">-{d.value}{d.type === 'percent' ? '%' : '$'}</span>
                                        <div className={`w-10 h-5 rounded-full relative transition-all cursor-pointer ${d.active ? 'bg-indigo-600' : 'bg-white/10'}`}>
                                            <div className={`absolute top-1 w-3 h-3 rounded-full bg-white transition-all ${d.active ? 'right-1' : 'left-1'}`}></div>
                                        </div>
                                    </div>
                                </div>
                            ))}
